...
pkgs <- c("readxl", "dplyr", "stringr", "lubridate", "tidytext", "syuzhet",
          "textdata", "ggplot2", "reshape2", "wordcloud", "writexl", "quanteda", "tidyr")
...
# Save cleaned data
cleaned_file <- file.path(save_dir, "gdelt_anglosphere_articles_with_body_cleaned.csv")
...
articles_long <- articles %>%
  ...
  mutate(keyword = strsplit(as.character(keyword), ",\\s*")) %>%
  unnest(keyword)
...
all_keywords <- unique(trimws(articles$keyword))
...
for (kw in all_keywords) {
  kwic_res <- kwic(toks, pattern = kw, window = 7, valuetype = "regex", case_insensitive = TRUE)
  if (nrow(kwic_res) > 0) {
    kwic_df <- as.data.frame(kwic_res)
    out_file <- file.path(save_dir, paste0("kwic_gdelt_", kw, ".csv"))
...
cat("Full GDELT Articles workflow (preprocessing + sentiment + visualization) complete.\n")
